function [xtrack,ztrack,xmtrack,zmtrack,zdiff,w,sdrift] = nls_stepper(K,Llx,ad,anl,cg,k0,kval,Om,om,sig,ep,tf,dt)
    ...
    % Solve NLS equation in time
        ...
        for nn=1:nmax
            ...
            n2 = a1*wp.^2;
            wp = ep^2*n0 + 2*ep*real(wp.*exp(1i*(k0*X/ep+ Om*(t+dt)/ep^2)) + ep*n2.*exp(2*1i*(k0*X/ep+ Om*(t+dt)/ep^2)) );
